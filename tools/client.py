...
class Client(object):
    def __init__(self, host, port=0, auth=None, username=None, password=None,
                 protocol='http', verify_ssl=True, path=None, cert=None, bearer=None):
        if not port:
        ...
        if bearer:
            self.session.headers.update({'Authorization': 'Bearer {}'.format(bearer)})
    ...
    def rmdir(self, path, safe=False):
        ...
        self._send('DELETE', path, expected_codes, headers={'If-Match': 'is_dir'})
    ...
    def ls(self, remote_path='.'):
        headers = {'Depth': '1'}
        response = self._send('PROPFIND', remote_path, (200, 207, 301), headers=headers)
...
if __name__ == '__main__':
    webdav = connect(host="localhost",
            port=8080,
            bearer="eyJhbGciOiJIUzUxMiIsInR5cCI6IkpXVCJ9.eyJ1aWQiOiJlMjYxOTM4MC02MjE0LTQ4ZmQtOGI5MS0xMmU1YjYzMjgzNjAiLCJpYXQiOjE1MDQwOTU5NzQsImV4cCI6MTUwNDE4MjM3NCwiaXNzIjoiQ3Jvc3NDbG91ZCJ9.lLnpDdx-bkbWZxez7XnYPgLTWrYm6_URkOPw7DfdsUO4-X7tyJs5kwsUh4C1RMWHvNy3LYc30YxjDyRppg_2FA",
            protocol='http',
            path='/')

    webdav.mkdir('/samba.0/some_dir')
    webdav.upload('test.txt', 'samba.0/some_dir/Österreich.txt')
    webdav.download('/samba.0/some_dir/Österreich.txt', './test_download.txt')
    webdav.rmdir('/samba.0/some_dir')
